...
import com.ly.tinycalendar.listener.OnPagerChangeListener;
...
public class CalendarView extends ViewPager {
    ...
    private DateBean lastSingleDate = new DateBean();
    private SparseArray<HashSet<DateBean>> chooseDate;//记录多选时全部选中的日期
    ...
    public void init() {
        ...
        lastSingleDate.setSolar(getSingleDate().getSolar()[0], getSingleDate().getSolar()[1], getSingleDate().getSolar()[2]);

        addOnPageChangeListener(new SimpleOnPageChangeListener() {
            @Override
            public void onPageSelected(int position) {
                refreshMonthView(position,1);
     ...
     * @param position
     */
    private void refreshMonthView(int position,int type) {
        ...
        if(type==2){
            ...
            int[]temp = CalendarUtil.getCurrentDate();
            lastSingleDate.setSolar(temp[0],temp[1],temp[2]);

            monthView.refresh(lastSingleDate, flag);
     ...
     * @param date
     */
    public void setLastClickDay(DateBean date) {

        lastClickDate[0] = currentPosition;
        lastClickDate[1] = date.getSolar()[2];
...
//        }
    ...
    public void today() {
        ...
        if (destPosition == currentPosition) {
            refreshMonthView(destPosition,2);
        } else {
    ...
    private void toDestDate(int year, int month, int day) {
        ...
        if (destPosition == currentPosition) {
            //在当月进行日期跳转
            refreshMonthView(destPosition,1);
        } else {
            setCurrentItem(destPosition, false);
        }
    }
...
//        LogUtils.e("setSingleDate"+singleDate);
